...
class SearchAdapter(private val target: SearchTarget): ListAdapter<EventTrackDB, SearchAdapter.ViewHolder>(EventDiffCallback) {

    class ViewHolder(itemView: View): RecyclerView.ViewHolder(itemView) {
        fun bind(position: Int, tracks: ArrayList<EventTrackDB>, target: SearchTarget) {
        ...
        private fun getNavId(target: SearchTarget): Int {
            return when(target) {
                SearchTarget.HOME -> R.id.action_home_to_playerActivity
                SearchTarget.PLAYLIST -> R.id.action_search_to_playerActivity
                SearchTarget.MY_PLAYLIST -> R.id.action_my_playlist_to_playerActivity
            }
        }
    }
